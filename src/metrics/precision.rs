...
pub fn compute_precision_fnr(
    ...
    for &th in quantiles {
        ...
        if n_y_high == 0 {
            ...
            let tp = xb.iter().zip(yb.iter()).filter(|(&xi, &yi)| xi >= th && yi >= thy).count();
            let fn_val = xb.iter().zip(yb.iter()).filter(|(&xi, &yi)| xi >= th && yi < thy).count();
...
pub fn linear_interpolate(x: &[f64], y: &[f64], xi: f64, n: usize) -> Result<f64, InterpError> {
    if xi <= x[0] { return Ok(f64::NAN); }
    if xi >= x[n-1] { return Ok(y[n-1]); }
    let interp = Interp1d::new_sorted(x.to_vec(), y.to_vec())?;
    Ok(interp.interpolate(xi))
}
...
pub fn precision_at_robust(
    ...
    for &thy in quality_thresholds {
        ...
        let (thx, precision_at_q) = (|| -> Result<(f64, f64), interp1d::error::InterpError> {
